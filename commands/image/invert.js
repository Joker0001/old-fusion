...
module.exports.run = async (bot, message, args) => {
    ...
    jimp.read(member.user.avatarURL)
        .then(img => {
            img.invert();
            img.write('../../images/output/memberinvert.jpg', async function () {
                await message.channel.send('<:imagem:492036737019674644> **Sua imagem:**')
                await message.channel.sendFile('../../images/output/memberinvert.jpg');
                await fs.unlink('../../images/output/memberinvert.jpg');
            });
        })
        .catch(err => {
            console.error(err);
